...
| **CHAT-05** | todo | チャット永続化 & 履歴UI | **Blocker解消**: DB パスワード受領済み。<br>**Step 1**: Supabase スキーマ追加（`conversations`, `messages`）と RLS ポリシー作成。<br>**Step 2**: API 追加 — `/api/conversations` (一覧), `/api/conversations/[id]` (詳細) を新設。<br>**Step 3**: `/api/chat` の onFinish で保存処理を実装。<br>**Step 4**: サイドバーで会話一覧を表示し、クリックでメッセージを再読み込み（タイトルは先頭発話30–50文字 or 日付で自動生成）。<br>**Step 5**: 保存→取得の統合テストを1件追加。 |
