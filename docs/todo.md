...
| **BE-14** | todo | 月次レポート生成 API | **Step 1** (review): `monthly_report` テーブルマイグレーション + 型定義 + MockQuery 対応。RLS: 生徒=自分のみ/スタッフ=全件/書込=Service Role のみ。<br>**Step 2**: `POST /api/reports/monthly` を実装（Cron 認証 + 月末判定 + 全生徒ループ）。<br>**Step 3**: 生徒ごとの統計集計 + LLM 分析プロンプト作成。<br>**Step 4**: `REPORT_LLM_MODEL` / `REPORT_LLM_API_KEY` での LLM 呼び出し。<br>**Step 5**: `monthly_report` への結果保存（成功/失敗）。<br>**Step 6**: 完了通知メール送信（Resend）。<br>仕様: `docs/reports/monthly.md` |
